...
// PolicyPilot is a self-hosted product — customers bring their own API keys.
...
function buildSystemPrompt(employee) {
  return `You are PolicyPilot AI, an expert HR policy assistant for enterprise organizations.
  ...
  try {
    ...
    if (HAS_API_KEY && use_llm !== false) {
      try {
        ...
        return NextResponse.json({
          answer: text,
          source: localResponse.source || "PolicyPilot AI (LLM)",
