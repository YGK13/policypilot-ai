...
// ============================================================================
// SYSTEM STATUS PANEL — fetches /api/health and shows which services are
// configured. If DB is connected but tables are missing, renders an
// "Initialize Database" button that POSTs to /api/setup.
// ============================================================================

function ServiceRow({ label, ok, detail }) {
  return (
    <div className="flex items-center gap-3">
      <span className={`w-2 h-2 rounded-full flex-shrink-0 ${ok ? "bg-green-400" : "bg-amber-400"}`} />
      <span className="text-xs font-medium text-gray-700 w-28 flex-shrink-0">{label}</span>
      <span className={`text-xs ${ok ? "text-green-700" : "text-amber-700"}`}>{detail}</span>
    </div>
  );
}
...
function SystemStatus() {
  ...
  if (loading) {
    return (
      <div className="mb-6 bg-white border border-gray-200 rounded-xl p-4 animate-pulse">
    ...
    <div className={`mb-6 rounded-xl border p-4 ${allGood ? "bg-green-50 border-green-200" : "bg-amber-50 border-amber-200"}`}>
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-xs font-bold text-gray-700 uppercase tracking-wide">
          {allGood ? "✅ Infrastructure Ready" : "⚠️ Infrastructure Status"}
        </h3>
        {setup?.needsSetup && services?.database?.latencyMs !== null && (
          <button
            onClick={initDb}
            disabled={initializing}
            className="px-3 py-1.5 text-xs font-semibold text-white bg-brand-600 rounded-lg hover:bg-brand-700 transition-colors disabled:opacity-60 cursor-pointer"
          >
            {initializing ? "Initializing…" : "🗄️ Initialize Database"}
          </button>
        )}
      </div>
      <div className="space-y-1.5">
        <ServiceRow
          ...
          detail={
            services?.database?.ok
              ? `Connected — ${services.database.tableCount} tables (${services.database.latencyMs}ms)`
              : services?.database?.latencyMs !== null && services?.database?.latencyMs !== undefined
                ? `Connected but ${setup?.missingTables?.length} table(s) missing — click Initialize`
                : "DATABASE_URL not configured — add Neon from Vercel Marketplace"
          }
        />
        <ServiceRow
          label="AI / LLM"
          ...
          detail={
            services?.llm?.gateway
              ? "AI Gateway (OIDC) — full streaming enabled"
              : services?.llm?.directKey
                ? "Direct Anthropic key — set ANTHROPIC_API_KEY ✓"
                : "Not configured — add ANTHROPIC_API_KEY in Vercel env vars"
          }
        />
        <ServiceRow
          label="Auth (Clerk)"
          ok={services?.clerk?.ok}
          detail={
            services?.clerk?.ok
              ? `Configured${services.clerk.webhook ? " + webhook" : " (webhook not registered)"}`
        ...
        <ServiceRow
          label="File Storage"
          ok={services?.blob?.ok}
          detail={services?.blob?.ok ? "Vercel Blob connected" : "BLOB_READ_WRITE_TOKEN missing — file uploads disabled"}
        />
        <ServiceRow
          label="Email"
          ok={services?.resend?.ok}
          detail={services?.resend?.ok ? "Resend configured — escalation emails active" : "RESEND_API_KEY not set — email alerts disabled"}
        />
        <ServiceRow
          label="Payments"
          ok={services?.stripe?.ok}
          detail={services?.stripe?.ok ? "Stripe connected" : "STRIPE_SECRET_KEY not set — running in demo billing mode"}
        />
      </div>
      {setup?.needsSetup && setup?.missingTables?.length > 0 && (
  ...
  return (
    <div className="min-h-[calc(100vh-54px)] bg-gray-50 flex items-start justify-center py-8 px-6">
      <div className="w-full max-w-2xl">
        {/* ============ Infrastructure Status (step 0 only) ============ */}
        {step === 0 && <SystemStatus />}
