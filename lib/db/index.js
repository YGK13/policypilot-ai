...
export async function createTicket(orgId, ticket) {
  ...
  // -- was_auto_resolved / escalation_reason feed the Tier 1 ROI card and the
  //    Tier 2 escalation-reasons breakdown. Persist them explicitly at write
  //    time rather than deriving from status+routing later, so the analytics
  //    queries stay cheap and the values are stable across future routing
  //    logic changes. --
  const wasAutoResolved = ticket.wasAutoResolved === true
    || ticket.routing === "auto"
    || ticket.routing === "auto_enhanced";
  const result = await sql`
    INSERT INTO tickets (id, org_id, user_id, query, category, risk_score, routing, status, priority, assignee, department, state, flags, resolution, ai_response, ai_confidence, policy_id, was_auto_resolved, escalation_reason)
    VALUES (${ticket.id}, ${orgId}, ${ticket.userId || null}, ${ticket.query}, ${ticket.category}, ${ticket.riskScore}, ${ticket.routing}, ${ticket.status}, ${ticket.priority}, ${ticket.assignee}, ${ticket.department}, ${ticket.state}, ${JSON.stringify(ticket.flags || [])}, ${ticket.resolution}, ${ticket.aiResponse || null}, ${ticket.aiConfidence || null}, ${ticket.policyId || null}, ${wasAutoResolved}, ${ticket.escalationReason || null})
    RETURNING *
  `;
  return result[0];
}
